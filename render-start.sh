...
if [ "${KARAOKE_DEMUCS_ENABLED:-1}" = "1" ]; then
  ...
  elif [ -d "$LEGACY_PYDEPS_DIR" ] && command -v python3 >/dev/null 2>&1 && PYTHONPATH="$LEGACY_PYDEPS_DIR${PYTHONPATH:+:$PYTHONPATH}" python3 -c "import demucs" >/dev/null 2>&1; then
    export PYTHONPATH="$LEGACY_PYDEPS_DIR${PYTHONPATH:+:$PYTHONPATH}"
    DEMUCS_PYTHON="python3"
    echo "[render-start] Demucs detected in legacy .pydeps via PYTHONPATH"
